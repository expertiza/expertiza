module StudentTeamsHelper
  ...
  def user_has_topic?(user_id, assignment_id)
    # This query takes the user_id and assignment_id as input then does a inner join between the Sign_up_Topicsm Signed_up_teams,
    # and teams_users table to find the if the user has selected any topic for that perticular assignmnet or not.
