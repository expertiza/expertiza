class User < ApplicationRecord
  ...
  def get_user_list(username_search = '', fullname_search = '', email_search = '')
    ...
    # regex for username, fullname and email generated based on the search criteria entered and then compare it to user list
    # Generates a regex for the searched username, fullname and email to compare it from user in the user list
    username_regex = generate_regex(username_search)
    fullname_regex = generate_regex(fullname_search)
    email_regex = generate_regex(email_search)

    # Results generated by each of the above regex matching criteria by AND operator combined into final_users
    # Return the results which satisfy the above result with specific condition by matching it with the user_list generated before.
