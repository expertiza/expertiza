class CourseTeam < Team
  ...
  def self.create(course_id)
    ...
    team_name = Team.generate_team_name(course.name)
    team = CourseTeam.create(:name => team_name, :parent_id => course_id)
  ...
  def export_participants
    ...
    participants.each do |participant|
      user_names.push(participant.name)
      user_names.push(' ')
    end
    return user_names
  end
  ...
  def self.export_all_assignment_team_related_to_course(csv, parent_id, options)
    course = Course.find(parent_id)
    assignment_list = Assignment.where(course_id: parent_id)
    assignment_list.each do |currentAssignment|
