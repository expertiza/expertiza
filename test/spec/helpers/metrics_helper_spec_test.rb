...
RSpec.describe MetricsHelper, type: :helper do
  ...
  describe "MetricsHelper Chart methods" do
    ...
    describe "#display_github_metrics" do
      it "returns a javascript function rendering the barchart timeline when passed github data" do
        expect(display_github_metrics(@parsed_data, @authors, @dates))
          .to eq('<canvas id="chart-0" class="chart" width="100" height="100"></canvas><script nonce="true">
//<![CDATA[
...
//]]>
    ...
    describe "#display_totals_piechart" do
      it "returns a GoogleCharts API link to a totals piechart when passed github data" do
        expect(display_totals_piechart(@parsed_data, @authors, @dates))
          .to eq('http://chart.apis.google.com/chart?chs=600x300&cht=p&chco=ff0000,ffff00&chd=s:9f&chl=student1+(25)|student2+(13)&chtt=%23+Commits+By+Author')
      end
    end
  end
end
