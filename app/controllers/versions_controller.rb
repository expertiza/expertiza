...
class VersionsController < ApplicationController
  ...
  def paginate_list
    ...
    if current_user_has_super_admin_privileges?
      if params[:post][:user_id].to_i > 0
        versions = versions.where(whodunnit: params[:post][:user_id])
      end
    end
    if current_user.try(:id).to_i > 0
      versions = versions.where(whodunnit: current_user.try(:id))
    end
    if params[:post][:item_type] && params[:post][:item_type] != 'Any'
      versions = versions.where(item_type: params[:post][:item_type])
    end
    if params[:post][:event] && params[:post][:event] != 'Any'
      versions = versions.where(event: params[:post][:event])
    end
