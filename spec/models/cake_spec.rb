describe 'cake' do
  ...
  let(:cake) { Cake.new id: 1, type: 'Cake', seq: 1.0, txt: 'Cake type item?', weight: 1, itemnaire: itemnaire, size: '50' }
  ...
  describe '#edit' do
    it 'returns the html ' do
      ...
      expect(html).to eq('<tr><td align="center"><a rel="nofollow" data-method="delete" href="/items/1">Remove</a></td><td><input size="6" value="1.0" name="item[1][seq]" id="item_1_seq" type="text"></td><td><textarea cols="50" rows="1" name="item[1][txt]" id="item_1_txt" placeholder="Edit item content here">Cake type item?</textarea></td><td><input size="10" disabled="disabled" value="Cake" name="item[1][type]" id="item_1_type" type="text"></td><td><input size="2" value="1" name="item[1][weight]" id="item_1_weight" type="text"></td><td>text area size <input size="3" value="50" name="item[1][size]" id="item_1_size" type="text"></td></tr>')
    end
  end

  describe '#view_item_text' do
    it 'returns the html ' do
      html = cake.view_item_text.to_s
      expect(html).to eq('<TR><TD align="left"> Cake type item? </TD><TD align="left">Cake</TD><td align="center">1</TD><TD align="center">0 to 5</TD></TR>')
    end
  end

  describe '#view_completed_item' do
    it 'returns the html ' do
      html = cake.view_completed_item(0, answer).to_s
      expect(html).to eq('<b>0. Cake type item?</b><div class="c5" style="width:30px; height:30px; border-radius:50%; font-size:15px; color:black; line-height:30px; text-align:center;">45</div><b>Comments:</b>')
    end
  end
  ...
  describe '#get_total_score_for_item' do
    context 'when the review is a Teammate Review Response Map' do
      it 'returns the scores of the team' do
        ...
        expect(cake.get_total_score_for_item('TeammateReviewResponseMap', 1, 1, 1, 1)).to eq(95)
      end
    end
    context 'when the item is not a teammate review response' do
      it 'returns zero' do
        arr = [answer, answer1]
        allow(Team).to receive(:joins).with([:teams_users, teams_users: [{ user: :participants }]]).and_return(arr)
        allow(arr).to receive(:where).with('participants.id = ? and teams.parent_id in (?)', 1, 1).and_return([team])
        allow(cake).to receive(:get_answers_for_teammatereview).with(1, 1, 1, 1, 1).and_return(arr)
        expect(cake.get_total_score_for_item('NotTeammateReviewResponseMap', 1, 1, 1, 1)).to eq(nil)
      end
    end
  end
  ...
  describe '#get_answers_for_teammatereview' do
    it 'gets an array of answers' do
      ...
      allow(Answer).to receive(:joins).with([{ response: :response_map }, :item]).and_return(arr1)
      allow(arr1).to receive(:where).with("response_maps.reviewee_id in (?) and response_maps.reviewed_object_id = (?)
      and answer is not null and response_maps.reviewer_id in (?) and answers.item_id in (?) and response_maps.reviewee_id not in (?)", [1], 1, 1, 1, 1).and_return([answer, answer1])
