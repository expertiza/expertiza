class ReviewMappingsController < ApplicationController
  ...
  def add_calibration
    ...
    redirect_to controller: 'responses', action: 'new', id: map.id, assignment_id: params[:id], return: 'assignment_edit'
  end
  ...
  def assign_reviewer_dynamically
    ...
    redirect_to controller: 'student_reviews', action: 'list', id: participant.id
  end
  ...
  def assign_metareviewer_dynamically
    ...
    redirect_to controller: 'student_reviews', action: 'list', id: metareviewer.id
  end
  ...
  def delete_metareviewer
    ...
    begin
      ...
      flash[:error] = "A delete action failed:<br/>" + $ERROR_INFO.to_s + "<a href='/review_mappings/delete_metareview/" + mapping.map_id.to_s + "'>Delete this mapping anyway>?"
    end
  ...
  def automatic_review_mapping_strategy(assignment_id,
    ...
    if student_review_num != 0 and submission_review_num == 0
      review_strategy = ReviewMappingsHelper::StudentReviewStrategy.new(participants, teams, student_review_num)
    elsif student_review_num == 0 and submission_review_num != 0
      review_strategy = ReviewMappingsHelper::TeamReviewStrategy.new(participants, teams, submission_review_num)
    end
