class TagPromptsDeployment < ActiveRecord::Base
  ...
  def get_assignment_tagging_progress()
    ...
    if not (teams.empty? or questions.empty?)
      teams.each do |team|
        responses = Response.joins("JOIN response_maps ON response_maps.id = responses.map_id").where(response_maps: {reviewed_object_id: self.assignment.id, reviewee_id: team.id})
        responses_ids = responses.map { |r| r.id }
