class PasswordRetrievalController < ApplicationController
  ...
  def send_password
    #Checking if email and user parameters are provided
    if params[:user][:email].nil? || params[:user][:email].strip.empty?
      flash[:error] = "Please enter an e-mail address."
    else
      #Authenticating user parameters
      user = User.find_by(email: params[:user][:email])
      if user
	#After user authentication changing the url format and send a secure token
        ...
        #Sending the reset password token to user
