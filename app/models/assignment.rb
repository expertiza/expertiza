...
require 'analytic/assignment_analytic'
  ...
  def add_participant(user_name, special_role,can_submit,can_review,can_take_quiz)
    ...
    if participant
      ...
      new_part = AssignmentParticipant.create(:parent_id => self.id, :user_id => user.id, :permission_granted => user.master_permission_granted, :special_role => special_role, :can_submit => can_submit, :can_review => can_review, :can_take_quiz => can_take_quiz)
      new_part.set_handle()
    end
  end
