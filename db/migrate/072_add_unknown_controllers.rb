class AddUnknownControllers < ActiveRecord::Migration
  def self.up
    ...
    controller = SiteController.find_or_create_by_name('student_teams')
    controller.permission_id = permission.id
    controller.save
    
    controller = SiteController.find_or_create_by_name('invitation')
    controller.permission_id = permission.id
    controller.save  
    
    permission = Permission.find_by_name('administer assignments')
    controller = SiteController.find_or_create_by_name('survey')
    controller.permission_id = permission.id
    controller.save     
  end
