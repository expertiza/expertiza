class QuestionnaireController < ApplicationController
  ...
  def save_new_questions(questionnaire_id)
    if params[:new_question]
      ...
      for question_key in params[:new_question].keys
        puts "Type"
        puts params[:new_question][question_key][:true_false]
        type = params[:new_question][question_key][:true_false]
        ques = params[:new_question][question_key]
        if(ques[:true_false] == "checkbox" || ques[:true_false] == "radio" || ques[:true_false] == "false")
          ques[:true_false] = false;
          if(type!="false")
           ques[:question_type] = type;
          end
        else
          ques[:true_false] = true;
        end
        q = Question.new(ques)
        ...
        if(type =="checkbox")
          for option_key in params[:checkbox_option][question_key].keys
            puts "Question"
            puts params[:checkbox_option][question_key][option_key]
            o = QuestionOption.new()
            o.option_text = params[:checkbox_option][question_key][option_key][:option_text]
            o.questions_id = q.id
            o.save if !o.option_text.strip.empty?
          end
        end
        if(type =="radio")
          for option_key in params[:radio_option][question_key].keys
            puts "Radio Question"
            puts params[:radio_option][question_key][option_key]
            o = QuestionOption.new()
            o.option_text = params[:radio_option][question_key][option_key][:option_text]
            o.questions_id = q.id
            o.save if !o.option_text.strip.empty?
          end
        end
  ...
  def save_questions(questionnaire_id)
    ...
    puts "Inside save question"
    puts questionnaire_id
    if params[:question]
      for question_key in params[:question].keys
        begin
          options = QuestionOption.find_all_by_questions_id(question_key)
          for option in options
            option.destroy
          end
          if params[:question][question_key][:txt].strip.empty?
            ...
            type = params[:question][question_key][:question_type]
            puts "Type"
            puts type
            if( type != "true")
              params[:question][question_key][:true_false] = false
            else
             params[:question][question_key][:true_false] = true
            end
            question = params[:question][question_key]
            question[:question_type] = type
            Question.update(question_key, question)
            if type == "checkbox"
              for option_key in params[:checkbox_option][question_key].keys
                o = QuestionOption.new()
                o.option_text = params[:checkbox_option][question_key][option_key][:option_text]
                o.questions_id = question_key
                o.save if !o.option_text.strip.empty?
              end
            end
            if type == "radio"
