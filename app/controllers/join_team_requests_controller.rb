...
class JoinTeamRequestsController < ApplicationController
  ...
  def check_team_status
    ...
    team_member = TeamsParticipant.find_by_team_id_and_user_id(params[:team_id], session[:user][:id])
    team = Team.find(params[:team_id])
    return flash[:error] = 'This team is full.' if team.full?
    return flash[:error] = 'You are already a member of this team.' unless team_member.nil?
  end
