...
def create_global_itemnaire(survey_name)
...
def deploy_global_survey(start_date, end_date, survey_name)
  ...
  create_global_itemnaire survey_name
...
describe 'Global Survey itemnaire tests for instructor interface' do
  before(:each) do
    course_setup
  ...
  it 'is able to delete item from a global survey' do
    survey_name = 'Global Survey Questionnaire 1'
    deploy_global_survey(@next_day, @next_to_next_day, survey_name)
    survey_itemnaire = Questionnaire.where(name: survey_name).first
    visit '/itemnaires/' + survey_itemnaire.id.to_s + '/edit'
