module ReviewMappingHelper
  ...
  def check_submission_state(response_map, assignment_created, assignment_due_dates, round, color)
    if submitted_within_round?(round, response_map, assignment_created, assignment_due_dates)
      ...
      if link.nil? or (link !~ %r{https*:\/\/wiki(.*)}) or (link !~ %r{https*:\/\/github(.*)}) # can be extended for github links in future
        color.push 'green'
      else
        color.push 'red'
      end
  ...
  def team_reviewed_link_name(max_team_size, response, reviewee_id, ip_address)
  ...
  def review_and_feedback_response_map(author)
