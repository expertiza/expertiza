describe PopupController do
  ...
  let(:itemnaire) { build_stubbed(:itemnaire, id: 1, max_item_score: 20) }
  let(:item) { build_stubbed(:item, id: 1, itemnaire_id: itemnaire.id) }
  let(:answer) { build_stubbed(:answer, id: 1, item_id: item.id, response_id: response.id, answer: 15) }
  ...
  final_versions = {
    "review round 1": { itemnaire_id: nil, response_ids: [] },
    "review round 2": { itemnaire_id: nil, response_ids: [] }
  }
  ...
  describe '#author_feedback_popup' do
    ...
    context 'when response_id exists' do
      it 'passes tests in first block' do
        allow(Question).to receive_message_chain(:find, :itemnaire_id).with(answer.item_id).with(no_args).and_return(itemnaire.id)
        allow(Questionnaire).to receive(:find).with(itemnaire.id).and_return(itemnaire)
        allow(itemnaire).to receive(:max_item_score).and_return(20)
        ...
        allow(response).to receive(:aggregate_itemnaire_score).and_return(20)
  ...
  describe '#team_users_popup' do
    it 'renders the page successfully as Instructor' do
      ...
      answer = double('answer', item_id: 1)
      ...
      allow(Response).to receive_message_chain(:aggregate_itemnaire_score).with(no_args).and_return(response)
  ...
  describe '#self_review_popup' do
    context 'when current user is the participant' do
      it 'render page successfully as Instructor to get maximum_score ' do
        item1 = double(:item1, itemnaire_id: 1)
        itemnaire1 = double(:itemnaire1, min_item_score: 1, max_item_score: 3, name: 'Test itemnaire')
        answer1 = double('Answer', item_id: 1)
        ...
        allow(Question).to receive(:find).with(1).and_return(item1)
        ...
        allow(Response).to receive_message_chain(:find, :aggregate_itemnaire_score).with('1').with(no_args).and_return(response1)
        allow(Response).to receive_message_chain(:find, :maximum_score).with('1').with(no_args).and_return(response1)
        request_params = { response_id: 1, user_fullname: itemnaire1.name }
        user_session = { user: instructor }
