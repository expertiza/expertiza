class PopupController < ApplicationController
  ...
  def participants_popup
    ...
    if (params[:response_map_id] == nil)
      ...
      @reviewid = (Response.find_by_map_id(params[:response_map_id])).id
      @pid = ResponseMap.find(params[:response_map_id]).reviewer_id
      @reviewer_id = Participant.find(@pid).user_id
      @assignment_id = ResponseMap.find(params[:response_map_id]).reviewed_object_id
      ...
      # @maxscore can be 0 if @review_questionnaire_id is nil or false
      if (@maxscore != 0)
        @sum1 = (100 * @sum.to_f) / (@maxscore * @count)
      end
    end
  end
  ...
  private
  @allowed_actions = [%w(team_users_popup), %w(participants_popup), %w(view_review_scores_popup), %w(reviewer_details_popup)]
end
