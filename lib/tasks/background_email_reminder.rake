...
task send_email_reminders: :environment do
  assignments = Assignment.find(:all, include: { participants: :user }, conditions: ['created_at >= ? AND created_at <= ?', Time.now - 1_209_600, Time.now])
  assignments.each do |assign|
    # fetching all due dates for the current assignment
    due_dates = DueDate.find(:all,
                             conditions: ['assignment_id = ?', assign.id])
    next if due_dates.empty? # making sure that the assignmefnt does have due dates
    ...
    due_dates.each do |date|
      next unless (date.due_at - Time.now) <= date.threshold * 3600 && (date.due_at - Time.now) > 0 && date.flag == false # send reminder

      deadlinetype = date.deadline_type_id
      submission_reminder(assign, date) if deadlinetype == 1 # 1 is submission
      review_reminder(assign, date) if deadlinetype == 2 # 2 is review
      metareview_reminder(assign, date) if deadlinetype == 5 # 5 is for metareview
      date.set_flag
    end # end of for loop
      ...
      response_maps.each do |response_map|
        ...
        unless response.nil? || resubmission_times.size <= 0 # meaning the reviewer has submitted a response for that map_id
          if response.updated_at < resubmission_times[0].resubmitted_at # participant/reviewer has reviewed an older version
            ...
            if !resubmission_times.empty?
              emails << email
            else
              # if the reviewee has made some sort of submission
  ...
  def metareview_reminder(assign, due_date)
    ...
    participants.each do |participant|
      ...
      response_maps.each do |response_map|
        ...
        next if check_response_map.empty?

        emails << email if response_map.response.empty?
      end
