class GroupUserNode < Node
  belongs_to :node_object, class_name: 'GroupsUser'
  ...
  def get_name
    GroupsUser.find(self.node_object_id).name
  end
  ...
  def self.get(parent_id)
    ...
    query = query + " and " + self.table + ".group_id = " + parent_id.to_s if parent_id
    find_by_sql(query)
  end
