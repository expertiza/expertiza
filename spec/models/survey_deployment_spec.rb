...
describe 'SurveyDeployment' do
  let(:assgt_survey_itemnaire) { AssignmentSurveyQuestionnaire.new id: 986, name: 'assgt_survey', private: 0, min_item_score: 0, max_item_score: 10, instructor_id: 1234 }
  ...
  it 'should not be valid if start time is missing' do
    survey_deployment = CourseSurveyDeployment.new itemnaire_id: 985, start_date: nil, end_date: next_day, parent_id: '12345678', type: 'CourseSurveyDeployment'
    expect(survey_deployment).not_to be_valid
  end

  it 'should not be valid if end time is missing' do
    survey_deployment = CourseSurveyDeployment.new itemnaire_id: 985, start_date: previous_day, end_date: nil, parent_id: '12345678', type: 'CourseSurveyDeployment'
    expect(survey_deployment).not_to be_valid
  end

  describe '#response_maps' do
    it 'should return the associated response map' do
      survey_deployment = AssignmentSurveyDeployment.new(itemnaire_id: 986, start_date: previous_day, end_date: nil, parent_id: '12345678', type: 'AssignmentSurveyDeployment', id: 1)
