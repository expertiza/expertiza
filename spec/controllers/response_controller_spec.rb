  ...
  describe '#update' do
    context 'when something is wrong during response updating' do
      it 'raise an error and redirects to response#save page' do
        ...
        expect(response).to redirect_to('/response/save?error_msg=Your+response+was+not+saved.+Cause%3A189+ERROR%21&id=1&msg=&review%5Bcomments%5D=some+comments')
      end
    ...
    context 'when response is updated successfully' do
      it 'redirects to response#save page' do
        ...
        session = {user: instructor}
        post :update, params, session
        expect(response).to redirect_to('/response/save?error_msg=&id=1&msg=Your+response+was+successfully+saved.&review%5Bcomments%5D=some+comments')
      end
    end
  end
