...
def fill_assignment_form()
  ...
  select('assignment_test_Course' , from: 'assignment_form_assignment_course_id')
...
#     # create(:topic, topic_name:'assignment_create_topic',assignment_id: @assignment[:id])
#     #
#     #
#     #
#     # (4..6).each do |i|
#     #   @student=User.find_by(name:'student206'+i.to_s)
#     #   @participant=create(:participant, assignment: @assignment4,user:@student)
#     #
#     # end
#
...
describe "creation page", js: true do
  ...
  it "is able to create a public assignment" do
    ...
    expect(assignment).to have_attributes(
                              ...
                              course_id: Course.find_by_name('assignment_test_Course' )[:id],
    ...
    Assignment.where(name:'public assignment for test').destroy_all
  end
  ...
  it "is able to create a private assignment" do
    ...
    Assignment.where(name:'private assignment for test').destroy_all
...
describe "topics tab", js: true do
  before(:each) do
    ...
    @assignment = Assignment.where(name: 'assignment_creation_test').first
    login_as("instructor6")
    visit "/assignments/#{@assignment[:id]}/edit"
    click_link 'Topics'
  end
  ...
  it "Add new topic and delete" do
    old_topics = SignUpTopic.count(:all, assignment_id: @assignment[:id])
    click_link 'New topic'
    click_button 'OK'
    fill_in 'topic_topic_identifier', with: '1'
    fill_in 'topic_topic_name', with: 'assignment_tmp_topic'
    fill_in 'topic_category', with: 'Test Category'
    fill_in 'topic_max_choosers', with: 2
    click_button 'Create'

    sign_up_topics = SignUpTopic.where(topic_name: 'assignment_tmp_topic').first
    expect(sign_up_topics).to have_attributes(
                                  topic_name: 'assignment_tmp_topic',
                                  max_choosers: 2,
                                  topic_identifier: '1',
    ...
    visit "/assignments/#{@assignment[:id]}/edit"
    click_link 'Topics'
    all("img[title='Delete Topic']")[0].click
    click_button 'OK'
    topics_exist = SignUpTopic.count(:all, assignment_id: @assignment[:id])
    expect(topics_exist).to be_eql old_topics
  end
...
describe "rubrics tab", js: true do
  before(:each) do
    @assignment = Assignment.find_by(name: 'Rubric_tab_test')
    login_as("instructor6")
    visit "/assignments/#{@assignment.id}/edit"
    click_link 'Rubrics'
  end
...
describe "review strategy tab", js: true do
  before(:each) do
    @assignment_id = Assignment.where(name: 'assignment_creation_test').first.id
  end
...
describe "participants", js: true do
  ...
  describe "participants", js: true do
    ...
    it "check to see if participants can be added" do
      student=User.find_by(name:'student1')
      login_as('instructor6')
      ...
      Participant.where(parent_id:assignment_id).delete_all
    end

    it "should display newly created assignment" do
      student=User.find_by(name:'student1')
      assignment = Assignment.where(name: 'participants Assignment').first

      create(:participant,user:student,assignment:assignment)
      login_as(student.name)
      expect(page).to have_content("participants Assignment")
      Participant.where(parent_id:assignment.id).delete_all
    end
  end
end
describe "Due dates tab", js: true do
  ...
  it "set the deadline for an assignment review" do
    ...
    fill_in 'datetimepicker_submission_round_1', with: '2020/10/01 12:00'
    fill_in 'datetimepicker_review_round_1', with: '2020/10/10 12:00'
    click_button 'submit_btn'

    submission_type_id = DeadlineType.where(name: 'submission')[0].id
    review_type_id = DeadlineType.where(name: 'review')[0].id

    submission_due_date = DueDate.where(parent_id:@assignment.id,deadline_type_id:DeadlineType.where(name: 'submission').first.id).first
    review_due_date = DueDate.where(parent_id:@assignment.id,deadline_type_id:DeadlineType.where(name: 'review').first.id).first
...
describe "check to find if the assignment can be added to a course", js: true  do
  it "assignment can be added to a course" do
    course_id = Course.where(name: 'assignment_test_Course')[0].id

    assignment= Assignment.where(name: 'assignment_creation_test')[0]

    login_as('instructor6')
    visit "/assignments/associate_assignment_with_course?id=#{assignment.id}"

    choose "course_id_#{course_id}"
    click_button 'Save'

    assignment_row = Assignment.where(name: 'assignment_creation_test')[0]
