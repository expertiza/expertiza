class Cake < ScoredQuestion
  ...
  # method is called during creation of itemnaire --> when cake type is added to the itemnaire.
  def edit(_count)
  ...
  # Method is called when completing the percentage contribution text box for a cake item in a review
  ...
  # E2218: This Method returns the total cake score for each item.
  ...
  # @return total_scores - a hash with key as item_id and values as total score for each cake item
  def self.get_total_score_for_items(review_type, items, participant_id, assignment_id, reviewee_id)
    total_scores = {}
    items.each do |item|
      next unless item.instance_of? Cake

      total_score = item.get_total_score_for_item(review_type, item.id, participant_id, assignment_id, reviewee_id).to_s
      total_score = 0 if total_score.nil?
      total_scores[item.id] = total_score
    end
    total_scores
  end

  # Finds all teammates and calculates the total contribution of all members for the item
  def get_total_score_for_item(review_type, item_id, participant_id, assignment_id, reviewee_id)
    # get the reviewer's team id for the currently answered item
    ...
    if review_type == 'TeammateReviewResponseMap'
      answers_for_team_members = get_answers_for_teammatereview(team_id, item_id, participant_id, assignment_id, reviewee_id)
    end
    calculate_total_score(answers_for_team_members) unless answers_for_team_members.nil?
  end

  # Finds the scores for all teammates for this item
  def get_answers_for_teammatereview(team_id, item_id, participant_id, assignment_id, reviewee_id)
    # get the reviewer's team members for the currently answered item
    ...
    Answer.joins([{ response: :response_map }, :item]).where("response_maps.reviewee_id in (?) and response_maps.reviewed_object_id = (?)
      and answer is not null and response_maps.reviewer_id in (?) and answers.item_id in (?) and response_maps.reviewee_id not in (?)", team_members, assignment_id, participant_id, item_id, reviewee_id).to_a
  end

  # Sums up the scores given by all teammates that should be less than or equal to 100
  def calculate_total_score(item_answers)
    item_score = 0.0
