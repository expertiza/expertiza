module LeaderboardHelper
  ...
  def self.getCourseName(courseID)
    courseName = if courseID.nil? || courseID.zero?
                   "Unaffiliated Assignments"
                 else
                   Course.find(courseID).name
                 end
    courseName
  end
  ...
  def self.getAssignmentName(assignmentID)
    assignmentName = if !assignmentID or assignmentID == 0
                       "Unnamed Assignment"
                     else
                       Assignment.find(assignmentID).name
                     end
    assignmentName
  end
  ...
  def self.getUserName(requesterID, userID)
    ...
    userName = if user.try(:leaderboard_privacy) and requesterID != userID and !instructor
                 "*****"
  ...
  def self.userIsInstructor?(userID)
    ...
    instructor = false
    instructor = true if instructorRoles.index(user.role_id)
    instructor
  end
  ...
  def self.studentInWhichCourses(studentUserId)
    ...
    courseList = assignmentRecords.uniq.pluck(:course_id)
  end
