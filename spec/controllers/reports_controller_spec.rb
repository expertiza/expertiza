    ...
    describe 'calibration' do
      context 'when type is Calibration and participant variable is nil' do
        it 'renders response_report page with corresponding data' do
          ...
          assignment_itemnaire = double('AssignmentQuestionnaire')
          allow(AssignmentQuestionnaire).to receive(:retrieve_itemnaire_for_assignment).with('1').and_return([assignment_itemnaire])
          allow(assignment_itemnaire).to receive_message_chain(:itemnaire, :items)
