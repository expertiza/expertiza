...
jQuery(document).ready(function($) {
    ...
    var disableFormEditorFields = function(fld)
    {
		...
		// add data for text area in criterion
		if($(fld).attr('type') == 'select'){
			$('.className-wrap').find('label').text('Textarea size');
			$('.className-wrap').find('.input-wrap').find('input').attr('placeholder', 'textarea size');
		}


		// remove certain fields in the form depending on the field type
		var _dropUl = $('ul.frmb-control');
		$(_dropUl).find('li[label="Radio Group"]').find('span').text('Scale');
		$(_dropUl).find('li[label="Select"]').find('span').text('Criterion');
    };

	var fieldReverseMap = {"radio-group": "Scale", "select": "Criterion", "checkbox": "Checkbox",
	...
	var onAddHandlerForCriterion = {
		onadd: function(fld)
		{
			...
			// add text area
			console.log($(fld).find('.prev-holder'));
			$(fld).append($('<textarea/>').css({'width': '50%', height: '50px'}));
		}
	};
	...
	fbOptions = {
	    ...
	    typeUserEvents: {
			...
			'select': onAddHandlerForCriterion,
			'text': onAddHandler,
...
function updateHiddenFields(li, type, tr){
	...
	switch(type){
		...
		case "select":{
			var _question = $(_form).find('.label-wrap').find('input[type="text"]').val();
			var _size = $(_form).find('.className-wrap').find('input[type="text"]').val();
			...
			$(_q).find('textarea').val(_question);
			$(_s).find('input[type="text"]').val(_size);
			$(f1).val(max_label);
			$(f2).val(min_label);
			break;
		}
