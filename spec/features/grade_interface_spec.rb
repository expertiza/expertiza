  ...
  context 'when checking grade bar chart' do
    ...
    let!(:review_response_map1) {
      (1..2).each do |assignment_id|
        ...
        (1..2).each do |team_id|
          2.times do
            create :review_response_map, reviewee: AssignmentTeam.find(team_id), reviewer: AssignmentParticipant.find(participant_id), assignment: Assignment.find(assignment_id)
            participant_id -= 1
          end
        end
      end}
