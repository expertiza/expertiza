describe SignupSheetsController do
  ...
  describe '#new' do
    it 'builds a new sign up topic and renders signup_sheets#new page' do
      params = {id: 1}
      get :new, params
  ...
  describe '#create' do
    context 'when topic cannot be found' do
      context 'when new topic can be saved successfully' do
        it 'sets up a new topic and redirects to assignment#edit page' do
          ...
          allow_any_instance_of(SignupSheetsController).to receive(:undo_link)
      ...
      context 'when new topic cannot be saved successfully' do
        it 'sets up a new topic and renders signup_sheets#new page' do
          allow(SignUpTopic).to receive(:where).with(topic_name: 'Hello world!', assignment_id: '1').and_return([nil])
          allow_any_instance_of(SignupSheetsController).to receive(:undo_link)
            .with("The topic: \"Hello world!\" has been created successfully. ").and_return('OK')
          allow(topic).to receive(:save).and_return('OK')
    ...
    context 'when topic can be found' do
      it 'updates the existing topic and redirects to signup_sheets#add_signup_topics_staggered page' do
        ...
        expect(response).to redirect_to('/signup_sheets/add_signup_topics_staggered?id=1')
      end
    end
  end
  ...
  describe '#edit' do
    it 'renders signup_sheets#edit page' do
      params = {id: 1}
      get :edit, params
  ...
  describe '#list' do
    ...
    context 'when current assignment is intelligent assignment and has submission duedate (deadline_type_id 1)' do
      it 'renders signup_sheets#intelligent_topic_selection page' do
        ...
        expect(response).to render_template('signup_sheets/intelligent_topic_selection')
      end
    end

    context 'when current assignment is not intelligent assignment and has submission duedate (deadline_type_id 1)' do
      it 'renders signup_sheets#list page' do
  ...
  describe '#sign_up' do
    context 'when SignUpSheet.signup_team method return nil' do
      it 'shows an error flash message and redirects to signup_sheets#list page' do
        ...
        expect(response).to redirect_to('/signup_sheets/list?id=1')
      end
    end
  end
  ...
  describe '#delete_signup' do
    ...
    context 'when either submitted files or hyperlinks of current team are not empty' do
      it 'shows a flash error message and redirects to signup_sheets#list page' do
    ...
    context 'when both submitted files and hyperlinks of current team are empty and drop topic deadline is nil' do
      it 'shows a flash success message and redirects to signup_sheets#list page' do
        allow(team).to receive(:submitted_files).and_return([])
        allow(team).to receive(:hyperlinks).and_return([])
        allow(SignedUpTeam).to receive(:find_team_users).with(1, 6).and_return([team])
        allow(team).to receive(:t_id).and_return(1)
  ...
  describe '#set_priority' do
    it 'sets priority of bidding topic and redirects to signup_sheets#list page' do
      allow(participant).to receive(:team).and_return(team)
      ...
      expect(response).to redirect_to('/signup_sheets/list?assignment_id=1')
    end
  end
  ...
  describe '#switch_original_topic_to_approved_suggested_topic' do
    it 'redirects to signup_sheets#list page' do
