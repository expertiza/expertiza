class StandardizeReviewOfReviewMappings < ActiveRecord::Migration  
  def self.up
    ...
    records = ApplicationRecord.connection.select_all("select * from `review_of_review_mappings`")
  ...
  def self.update_mapping(mapping)
      ...
      review = ApplicationRecord.connection.select_one("select * from `review_of_reviews` where mapping_id = #{mapping["id"]}")
      review_mapping = ApplicationRecord.connection.select_one("select * from `review_mappings` where id = #{mapping["reviewed_object_id"]}")
