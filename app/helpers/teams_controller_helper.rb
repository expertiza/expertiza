...
module TeamsControllerHelper
  ...
  def create_team(parent)
    ...
    @team = Object.const_get("#{session[:team_type]}Team").create_team_manually(name: params[:team][:name],
                                                                                parent_id: parent.id)
  end
  ...
  def undo_link_for_deletion(team_name)
  ...
  def copy_teams_to_course(teams, course)
    ...
    flash_note("#{teams.length} teams were successfully copied to \"#{course.name}\"")
  end

  def copy_teams_to_assignment(teams, assignment)
    Team.copy_content(teams, assignment)
    flash_note("#{teams.length} teams were successfully copied to \"#{assignment.name}\"")
  end
