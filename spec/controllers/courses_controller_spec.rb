describe CoursesController do
  ...
  describe '#action_allowed?' do
    ...
    context 'when current user is instructor' do
      it 'allows all courses actions' do
        user = instructor
        stub_current_user(user, user.role.name, user.role)
  ...
  describe '#create' do
    it 'redirects to tree_display#list page after creating a new courses' do
      expect { create(:course) }.to change(Course, :count).by(1)
    end
  end

  describe '#edit' do
    context 'when @courses is not nil' do
      it 'renders the courses#edit page' do
  ...
  describe '#delete' do
    it 'deletes the courses and redirects to tree_display#list page' do
