...
describe ReviewMappingHelper, type: :helper do
  ...
  # rspec test for get_review_metrics method
  describe 'get_review_metrics' do
    ...
    before(:each) do
      ...
      # Populate @avg_and_ranges instance variable required for lookup to extract the metrics for the given round in get_review_metrics method.
    ...
    it 'should return minimum maximum and average score for round 1' do
      @round = 1
      get_review_metrics(@round, @reviewee.id)
      expect(@min).to eq '2%'
      expect(@max).to eq '4%'
      expect(@avg).to eq '3%'
    end
