class AssignmentTeam < Team
  ...
  def self.get_first_participant(team_id)
  ...
  def reviewed_contributor?(contributor)
    !TeamReviewResponseMap.all(conditions: ['reviewee_id = ? && reviewer_id = ? && reviewed_object_id = ?', contributor.id, self.id, assignment.id]).empty?
  end
  ...
  def get_hyperlinks
    links = Array.new
    self.participants.each { |participant| links.concat(participant.get_hyperlinks_array) }
    links
  end
  ...
  def self.import(row, session, assignment_id, options)
    ...
    # insert participants into team unless team was pre-existing & we ignore duplicate teams
    team.import_participants(index, row) unless team_exists && options[:handle_dups] == 'ignore'
  end
  ...
  def self.export(csv, parent_id, options)
    ...
    current_assignment.teams.each do |team|
      ...
      if options["team_name"] == "false"
        participants = TeamsUser.all(conditions: ['team_id = ?', team.id])
