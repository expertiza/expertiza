  ...
  describe '#accept_invitation' do
    context 'a user is not on a team and wishes to join a team with open slots' do
      it 'places the user on a team and returns true' do
        ...
        expect(Invitation.accept_invitation(team_id, user2.id, user3.id, assignment_with_participants.id)).to eq(true)
      end
    end
  ...
  describe '#update_users_topic_after_invite_accept' do
    context 'the invited user was already in another team before accepting their invitation' do
      it 'updates their team user mapping' do
        ...
        allow(TeamsUser).to receive(:team_id).with(assignment.id, user3.id).and_return(team3.id)
        allow(TeamsUser).to receive(:find_by).with(team_id: team3.id, participant_id: participant3.id).and_return(created_teams_user)
        ...
        allow(Team).to receive(:find).with(team3.id).and_return(team3)
        allow(Assignment).to receive(:find).with(assignment_with_participants.id).and_return(assignment_with_participants)
        allow(assignment_with_participants).to receive(:participants).and_return(assignment_with_participants.participants)
        allow(assignment_with_participants.participants).to receive(:find_by).with(user_id: user3.id).and_return(participant3)
