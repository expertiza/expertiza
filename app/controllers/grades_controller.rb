class GradesController < ApplicationController
  ...
  def view_team
    ...
    if @assignment.is_selfreview_enabled?
      ...
      actual_score = Rscore.new(@pscore, :review).my_avg || 0
      # impact factor is the maximum bonus marks given to students
      impact_factor = 10
      # self _score ranges from 0 to 1, this will go nearer to 1 as avg_self_review_score goes nearer to actual_score
      self_score = 1 - ((actual_score-avg_self_review_score).abs/100)
      # Composite_final_score will add the bonus marks to actual_score. Bonus marks is generated by multiplying impact_factor with self_score
      composite_final_score = actual_score + (impact_factor * self_score)
      # new_derived_scores will be composite_score if reviews does not vary rubrics by rounds.
      if @assignment.varying_rubrics_by_round?
