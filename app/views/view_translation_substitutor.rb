...
class ViewTranslationSubstitutor
  ...
  def process_view(directory_name, view_name, translations)
    ...
    unless File.exists?(path)
      ...
      return "<file not found>" unless File.exists?(path)
    end
  ...
  def process_translation(contents, key, val)
    ...
    translation_stats = {}
    translation_stats["replacements"] = replacements if replacements.length > 0
    translation_stats["skips"] = skips if skips.length > 0
