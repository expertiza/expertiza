class User < ApplicationRecord
  ...
  # We added a method named get_user_list. This method basically generates the user list based on the search criteria entered. 
  ...
  def get_user_list(search_uname = '', search_fname = '', search_email = '')
    # First we create an empy user_list list in order to store user list
    user_list = []
    # If super admin, then fetch and display all the users
    user_list = SuperAdministrator.get_user_list if role.super_admin?

    # If an instructor, then fetch and display all the users in his particular course/assignment
    user_list = Instructor.get_user_list(self) if role.instructor?

    # If TA, then fetch and display all users in his courses
    ...
    # Now we generate the regex for username, fullname and email based on the search criteria entered and then compare it to user list
    # Generates a regex for the searched username, fullname and email to compare it from user in the user list
    regex_uname = Regexp.new(search_uname)
    regex_fname = Regexp.new(search_fname)
    regex_email = Regexp.new(search_email)

    # Now we combine the results generated by each of the above regex matching criteria by AND operator
    # Return the results which satisfy the above result with specific condition by maching it with the user_list generated before.
    # Store the results in select_users
