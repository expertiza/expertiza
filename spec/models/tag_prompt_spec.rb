...
describe TagPrompt do
  ...
  let(:an_long) { Answer.new item: ct_criterion, answer: 5, comments: 'test comments' }
  let(:an_long_text) { Answer.new item: ct_text, answer: 5, comments: 'test comments' }
  let(:an_cb) { Answer.new item: ct_cbox, answer: 1 }
  let(:an_short) { Answer.new item: ct_criterion, answer: 5, comments: 'yes' }
  ...
  let(:tag_dep) { TagPromptDeployment.new id: 1, tag_prompt: tp, item_type: 'Criterion', answer_length_threshold: 5 }
  let(:tag_dep_slider) { TagPromptDeployment.new id: 2, tag_prompt: tp2, item_type: 'Criterion', answer_length_threshold: 5 }
  ...
  it 'returns an empty string when the item_type is not Criterion' do
