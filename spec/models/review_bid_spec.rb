describe ReviewBid do
  ...
  describe '#bidding_data' do
    it 'checks if get_bidding_data returns bidding_data as a hash' do
      ...
      expect(ReviewBid.bidding_data).to eq(
        'max_accepted_proposals' => nil
        'tid' => [123, 124],
        'otid' => 123,
        'priority' => [3, 2],
        'time' => ['2018-01-01 00:00:00', nil]
      )
  ...
  describe '#assign_review_topics' do
    it 'calls assigns_topics_to_reviewer for as many topics associated' do
      maps = [response_map]
      matched_topics = { 7601 => [123], 7602 => [124] }
      allow(ReviewResponseMap).to receive(:where).and_return(maps)
      allow(response_map).to receive(:destroy_all).and_return(true)
  ...
  context 'when no team exists for the topic' do
    it 'does not create a review response map' do
        allow(SignedUpTeam).to receive_message_chain(:where, :pluck, :first).and_return(nil)
        expect(ReviewResponseMap).not_to receive(:create)
        bid1.assign_topic_to_reviewer(7601, 123)
      end
    end
    context 'when there is a team to review' do
      it 'creates a review response map' do
        ...
        expect(ReviewResponseMap).to receive(:create).with(
          reviewed_object_id: 2085,
          reviewer_id: 7601,
          reviewee_id: 2,
