class SuggestionsController < ApplicationController
  ...
  def create
    ...
    if @suggestion.save
      flash[:success] = 'Thank you for your suggestions!' if @suggestion.unityID != ''
      flash[:success] = 'You have submitted an anonymous suggestions. It will not show in the suggested topic table below.' if @suggestion.unityID == ''
    end
  ...
  # If the user submits a suggestions and gets it approved -> Send email
  # If user submits a suggestions anonymously and it gets approved -> DOES NOT get an email
  def send_email
  ...
  def reject_suggestion
    ...
    if @suggestion.update_attribute('status', 'Rejected')
      flash[:notice] = 'The suggestions has been successfully rejected.'
    else
      flash[:error] = 'An error occurred when rejecting the suggestions.'
    end
    redirect_to action: 'show', id: @suggestion
  end
  ...
  def approve
    ...
    if @signuptopic.save && @suggestion.update_attribute('status', 'Approved')
      flash[:success] = 'The suggestions was successfully approved.'
    else
      flash[:error] = 'An error occurred when approving the suggestions.'
    end
  end
end
