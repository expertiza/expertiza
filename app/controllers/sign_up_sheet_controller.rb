...
class SignUpSheetController < ApplicationController
    ...
    if user.nil? # validate invalid user
      ...
      if AssignmentParticipant.exists? user_id: user.id, parent_id: params[:assignment_id]
        if SignUpSheet.signup_team(params[:assignment_id], user.id, params[:topic_id])
          ...
          ExpertizaLogger.info LogMessage.new(controller_name, '', 'Instructor signed up student for topic: ' << params[:topic_id])
        else
          flash[:error] = "The student has already signed up for a topic!"
          ExpertizaLogger.info LogMessage.new(controller_name, '', 'Instructor is signing up a student who already has a topic')
        end
      else
        flash[:error] = "The student is not registered for the assignment!"
        ExpertizaLogger.info LogMessage.new(controller_name, '', 'The student is not registered for the assignment: ' << user.id)
      end
    end
  ...
  def delete_signup
    ...
    if !participant.team.submitted_files.empty? or !participant.team.hyperlinks.empty?
      ...
      ExpertizaLogger.error LogMessage.new(controller_name, session[:user].id, 'Student is trying to drop a topic for which (s)he has already submitted a work: ' << params[:topic_id])
      ...
      ExpertizaLogger.error LogMessage.new(controller_name, session[:user].id, 'Student is trying to drop a topic whose deadline has ended: ' << params[:topic_id])
    else
      ...
      ExpertizaLogger.info LogMessage.new(controller_name, session[:user].id, 'Student has dropped the topic: ' << params[:topic_id])
    end
    redirect_to action: 'list', id: params[:id]
  end
  ...
  def delete_signup_as_instructor
    ...
    if !participant.team.submitted_files.empty? or !participant.team.hyperlinks.empty?
      ...
      ExpertizaLogger.error LogMessage.new(controller_name, session[:user].id, 'Signup drop failed as student has already submitted their work: ' << params[:topic_id])
    elsif !drop_topic_deadline.nil? and Time.now > drop_topic_deadline.due_at
      flash[:error] = "You cannot drop a student after the drop topic deadline!"
      ExpertizaLogger.error LogMessage.new(controller_name, session[:user].id, 'Signup drop failed as topic deadline has passed: ' << params[:topic_id])
    else
      ...
      ExpertizaLogger.error LogMessage.new(controller_name, session[:user].id, 'Student has been dropped from the topic: ' << params[:topic_id])
    end
