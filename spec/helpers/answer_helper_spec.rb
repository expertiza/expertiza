  ...
  describe '#delete_existing_responses' do
    context 'when the response is in reviewing period' do
      it 'deletes the answers' do
        ...
        allow(AnswerHelper).to receive(:log_response_info).with([@answer.response_id]).and_return({@answer.response_id=>[@user.email, @answer.comment, @user.name, @assignment1.name]})
