class CreateResponseMaps < ActiveRecord::Migration
  def self.up
    ...
    ApplicationRecord.connection.select_all("select * from review_of_review_mappings").each{
       | map | 
       rmap = ApplicationRecord.connection.select_all("select * from review_mappings where id = #{map['reviewed_object_id']}")       
    ...
    ApplicationRecord.connection.select_all("select * from feedback_mappings").each{
       | map | 
       review = ApplicationRecord.connection.select_all("select * from reviews where id = #{map['reviewed_object_id']}")
       rmap = ApplicationRecord.connection.select_all("select * from review_mappings where id = #{review[0]['mapping_id']}")
       assignment = Assignment.find(rmap[0]['reviewed_object_id'].to_i)              
       create_response_map(map,"review_feedbacks","FeedbackResponseMap", "AuthorFeedbackQuestionnaire",assignment)
    }  
    
    ApplicationRecord.connection.select_all("select * from teammate_review_mappings").each{
       | map | 
    ...
    ApplicationRecord.connection.select_all("select * from review_mappings").each{
       | map | 
       
       assignment = Assignment.find(map['reviewed_object_id'].to_i) 
       
       review = ApplicationRecord.connection.select_all("select * from reviews where mapping_id = #{map['id']}")
  ...
  def self.create_response_map(map, review_type, map_type, questionnaire_type, assignment)
    ...
    oldest_allowed_time = Time.local(today.year - 1,today.month,today.day,0,0,0)    
       review = ApplicationRecord.connection.select_all("select * from #{review_type} where mapping_id = #{map['id']}")
