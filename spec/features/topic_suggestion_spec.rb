...
#   Case 1: One team is on the waitlist. They sent a suggestions for new topic and they want to choose their suggested topic. After their suggested topic is approved, they should leave the waitlist and hold their suggested topic;
...
describe "Assignment Topic Suggestion Test" do
  ...
  describe "topic_suggestion" do
    it "Instructor set an assignment which allow student suggest topic and register student2065" do
      ...
      click_button 'Submit'
      expect(page).to have_content "Thank you for your suggestions"
      logout
      visit login_path
      user = User.find_by(name: 'instructor6')
      login_as(user.name)

      # instructor approve the suggestions topic
      # DUE date need to be added here
      visit '/suggestions/list?id=1&type=Assignment'
      expect(page).to have_content "Assignment_suggest_topic"
      click_link('View')
      expect(page).to have_content "suggested_description"
      click_button 'Approve suggestions'
      expect(page).to have_content "The suggestions was successfully approved."
    end
    ...
    ########################################
    # Case 3:
    # One team is holding a topic. They sent a suggestions for new topic, and keep themselves in old topic
    ########################################
    it "student2065 hold suggest topic and suggest a new one, but wish to stay in the old topic", js: true do
      ...
      ######################################
      # One team is holding a topic. They sent a suggestions for new topic
      ######################################
      # login_as "student2065"
      user = User.find_by(name: 'student2065')
      login_as(user.name)
      ...
      visit '/signup_sheets/publish_approved_suggested_topic/2?assignment_id=1'
    ...
    it "professor could approve anonymous suggestions topic" do
      # login_as "student2064"
      user = User.find_by(name: 'student2064')
      login_as(user.name)
      visit '/student_tasks/list'
      ...
      click_button 'Submit'
      expect(page).to have_content "You have submitted an anonymous suggestions."
      click_link("Logout")
