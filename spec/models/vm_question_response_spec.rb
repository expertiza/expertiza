describe VmQuestionResponse  do
  let(:review_itemnaire) { build(:itemnaire) }
  let(:author_feedback_itemnaire) { AuthorFeedbackQuestionnaire.new }
  let(:teammate_review_itemnaire) { TeammateReviewQuestionnaire.new }
  let(:metareview_itemnaire) { MetareviewQuestionnaire.new }
  let(:assignment) { build(:assignment) }
  let(:item) { build(:item, id: 2, itemnaire: review_itemnaire, weight: 2, type: 'good') }
  let(:items) { [item] }
  ...
  let(:response) { VmQuestionResponse.new(review_itemnaire, assignment, 1) }
  ...
  let(:tag_dep) { TagPromptDeployment.new id: 1, tag_prompt: tp, tag_prompt_id: 1, item_type: 'Criterion', answer_length_threshold: 5, itemnaire: review_itemnaire, assignment: assignment }
  let(:tp) { TagPrompt.new(prompt: 'test prompt', desc: 'test desc', control_type: 'Checkbox') }

  describe '#initialize' do
    context 'when initialized with a review itemnaire' do
      it 'initializes the instance variables' do
        expect(response.round).to eq 1
        expect(response.rounds).to eq(2)
        expect(response.itemnaire_type).to eq('ReviewQuestionnaire')
        expect(response.itemnaire_display_type).to eq('Review')
    ...
    context 'when initialized with any other itemnaire type' do
      it 'initializes the instance variables' do
        response = VmQuestionResponse.new(metareview_itemnaire, assignment, 1)
        expect(response.round).to eq(1)
        expect(response.rounds).to eq(2)
        expect(response.itemnaire_display_type).to eq('Metareview')
        expect(response.list_of_rows).to eq([])
        expect(response.list_of_reviewers).to eq([])
        expect(response.list_of_reviews).to eq([])
      end
  ...
  describe '#add_items' do
    it 'adds items' do
      ...
      vm_item_response_row = response.list_of_rows.first
      expect(vm_item_response_row.class.to_s).to eq 'VmQuestionResponseRow'
      expect(vm_item_response_row.item_text).to eq('Test item:')
      expect(vm_item_response_row.item_id).to eq(2)
      expect(vm_item_response_row.weight).to eq(2)
      expect(vm_item_response_row.item_seq).to eq(1.00)
    end
  end
  ...
  describe '#add_reviews' do
    ...
    context 'when initialized with a author feedback itemnaire' do
      it 'adds reviews' do
        author_feedback_itemnaire.type = "AuthorFeedbackQuestionnaire"
        response = VmQuestionResponse.new(author_feedback_itemnaire, assignment, 1)
    ...
    context 'when initialized with a teammate review itemnaire' do
      it 'adds reviews' do
        teammate_review_itemnaire.type = "TeammateReviewQuestionnaire"
        response = VmQuestionResponse.new(teammate_review_itemnaire, assignment, 1)
    ...
    context 'when initialized with a meta review type' do
      it 'adds reviews' do
        metareview_itemnaire.type = "MetareviewQuestionnaire"
        response = VmQuestionResponse.new(metareview_itemnaire, assignment, 1)
  ...
  describe '#max_score_for_itemnaire' do
    context 'when there are no rows' do
      it 'return 0' do
        expect(response.max_score_for_itemnaire).to eq(0)
      end
    end
  end
  ...
  describe '#add_answer' do
    it 'returns a VmQuestionResponseScoreCell' do
      ...
      allow(Question).to receive(:find).and_return(item)
