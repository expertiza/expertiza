class ResponsesController < ApplicationController
  ...
  def action_allowed?
    ...
    case action
    when 'edit' # If responses has been submitted, no further editing allowed
  ...
  # GET /responses/json?response_id=xx
  def json
  ...
  def delete
    ...
    # The lock will be automatically destroyed when the responses is destroyed
    @response.delete
    redirect_to action: 'redirect', id: map_id, return: params[:return], msg: "The responses was deleted."
  end
  ...
  # Update the responses and answers when student "edit" existing responses
  def update
    render nothing: true unless action_allowed?
    
    msg = ""
    begin
      # the responses to be updated
    ...
    ExpertizaLogger.info LoggerMessage.new(controller_name, session[:user].name, "Your responses was submitted: #{@response.is_submitted}", request)
  ...
  def new
    ...
    # The responses must be created when the review begin.
    # So do the answers, otherwise the responses object can't find the questionnaire when the user hasn't saved his new review and closed the window.
    # A new responses has to be created when there hasn't been any reviews done for the current round,
  ...
  def new_feedback
    ...
    if review
      ...
      if map.nil?
        # if no feedback exists by dat user den only create for dat particular responses/review
  ...
  def create
    ...
    # Hence we need to pick the latest responses.
    ...
    @response.update(additional_comment: params[:review][:comments], is_submitted: is_submitted) # ignore if autoupdate try to save when the responses object is not yet created.

    # :version_num=>@version)
    # Change the order for displaying questions for editing responses views.
    questions = sort_questions(@questionnaire.questions)
    create_answers(params, questions) if params[:responses]
    msg = "Your responses was successfully saved."
    error_msg = ""
  ...
  def redirect
    ...
    when "survey"
      redirect_to controller: 'survey_deployments', action: 'pending_surveys'
    when "bookmark"
      ...
      redirect_to controller: 'student_reviews', action: 'list', id: reviewer_id
    end
  end
  ...
  def toggle_permission
    ...
    begin
      ...
      # Updating visibility for the responses object, by E2022 @SujalAhrodia -->
  ...
  # This action gets taken if the responses is locked and cannot be edit right now
  def response_lock_action
    redirect_to action: 'redirect', id: @map.map_id, return: 'locked', error_msg: 'Another user is modifying this responses or has modified this responses. Try again later.'
  end
  ...
  # if false: we figure out which questionnaire to display base on @responses object
  ...
  def set_questionnaire
    # if user is not filling a new rubric, the @responses object should be available.
