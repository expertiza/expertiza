class ResponseController < ApplicationController
  ...
  def new
    ...
    @questions.each do |question|
      if QuesCount.find_by_question_id_and_assignment_id(question.id,@assignment.id)
        question.count=QuesCount.find_by_question_id_and_assignment_id(question.id,@assignment.id).count
        sum_of_count=sum_of_count + question.count;
      else
        question.count=0
      end

    end
    size_of_ques=@questions.length
    avg=sum_of_count/size_of_ques
    @selected_ques=[]
    @questions.each { |question| @selected_ques << question if question.count <= avg }
    puts "-----------------------------------------"
    puts @selected_ques
    @questions=@selected_ques
    @questions=@questions.sort { |m1,m2|(m1.count and m2.count) ? m1.count <=> m2.count : (m1.count ? -1 : 1)}
  ...
  def create
    ...
    begin
      ...
      params[:responses].each_pair do |k,v|
        ...
        score = Score.create(:response_id => @response.id, :question_id => questions[k.to_i].id, :score => v[:score], :comments => v[:comment])
        if v[:comment]!=""
          @selected_question=QuesCount.find_by_question_id_and_assignment_id(questions[k.to_i].id,@map.reviewed_object_id)
          if @selected_question
            count_increment=@selected_question.count
            count_increment=count_increment+1
            @selected_question.update_attribute('count',count_increment)
          else
            quescount=QuesCount.create(:assignment_id=>@map.reviewed_object_id, :question_id=>questions[k.to_i].id, :count=>1)
          end
        end
      end
      # rescue
