...
# ============= yxue4,xfang2,hsun6================
    ...
    #sc_team = ScoreCache.find(:all, :conditions => [""])
    if ( sc == nil)
      presenceflag = 1
      @msgs = "first entry"
      sc = ScoreCache.new
    ...
    tm_rv_ct = ScoreCache.count(:all, :conditions => ["team_id = ? and object_type = ?", tm_id, "TeammateReviewResponseMap"])
    sc_proj = ScoreCache.find(:first, :conditions => ["team_id = ? and object_type = ?", tm_id, "TeamReviewResponseMap"])
    ...
    if tm_user_ct == tm_rv_ct and sc_proj != nil
      ...
      tm_rv_all = ScoreCache.find(:all, :conditions => ["team_id = ? and object_type = ?", tm_id, "TeammateReviewResponseMap"])
      ...
      tm_rv_all.each do |item|
        ...
        if teamrvsc < (threshold - offset)
          sc_mod = ScoreCache.find(:first, :conditions => ["team_id = ? and reviewee_id = ? and object_type = ?", tm_id, item.reviewee_id, "ModifiedAssignmentScore"])
          if sc_mod == nil
    ...
    score_all = ScoreCache.find(:all, :conditions => ["team_id = ? and object_type = ?", tm_id, "ModifiedAssignmentScore"])
