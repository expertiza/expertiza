module Api::V1
  class ReviewMappingController < BasicApiController
    # include GC4R
    ...
    def assign_reviewer_dynamically
      ...
      if params[:i_dont_care].nil? && params[:topic_id].nil? && assignment.topics? && assignment.can_choose_topic_to_review?
        @server_msg = "No topic is selected.  Please go back and select a topic."
      else
        # begin
        if assignment.topics? # assignment with topics
          ...
          if topic.nil?
            @server_msg = "No topics are available to review at this time. Please try later."
          else
          ...
          if assignment_team.nil?
            @server_msg = "No artifacts are available to review at this time. Please try later."
          else
            assignment.assign_reviewer_dynamically_no_topic(reviewer, assignment_team)
          end
        end
        end
       render json: {status: :ok, server_msg: @server_msg}
    end
