class SignedUpUser < ActiveRecord::Base
  ...
  def self.find_team_users(assignment_id,user_id)
    TeamsUser.find_by_sql("SELECT t.id as t_id FROM teams_users u, teams t WHERE u.team_id = t.id and t.parent_id =" + assignment_id.to_s + " and u.user_id =" + user_id.to_s)
  end
