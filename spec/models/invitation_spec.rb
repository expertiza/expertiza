describe Invitation do
  ...
  describe '#is_invited?' do
  	context 'an invitation has been sent between user1 and user2' do
  		it 'returns true' do
  			allow(Invitation).to_receive(:where).with('from_id = ? and to_id = ? and assignment_id = ? and reply_status = "W"',
                                       invitee_user_id, invited_user_id, assignment_id).and_return(Invitation.new)
  	...
  	context 'an invitation has not been sent between user1 and user2' do
  		it 'returns false' do
  			allow(Invitation).to_receive(:where).with('from_id = ? and to_id = ? and assignment_id = ? and reply_status = "W"',
                                       invitee_user_id, invited_user_id, assignment_id).and_return(nil)
