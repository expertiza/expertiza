class Assessment360Controller < ApplicationController
  ...
  def calc_aggregate_score(review)
    ...
    review.each do |cp_id, assignment_review_scores_map|
      aggregate_scores[cp_id] = (assignment_review_scores_map.inject(0) { |_sum, (_k, v)| _sum += v } * 1.0 / assignment_review_scores_map.size).round unless assignment_review_scores_map.empty?
    end
    aggregate_scores
  end
