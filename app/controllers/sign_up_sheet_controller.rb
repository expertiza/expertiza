...
class SignUpSheetController < ApplicationController
  ...
  def show_team
    ...
    if assignment && topic
      ...
      @results.each do |result|
        @team_members = ''
        TeamsParticipant.where(team_id: result[:team_id]).each do |teamparticipant|
          @team_members += User.find(teamparticipant.user_id).name + ' '
        end
      end
  ...
  def switch_original_topic_to_approved_suggested_topic
    assignment = AssignmentParticipant.find(params[:id]).assignment
    team_id = TeamsParticipant.find_team_id(assignment.id, session[:user].id)
