class Invitation < ApplicationRecord
  ...
  def self.update_users_topic_after_invite_accept(invitee_user_id, invited_user_id, assignment_id)
    ...
    if original_team_id
      ...
      team_user_mapping = TeamsUser.find_by_team_id_and_user_id(original_team_id, invited_user_id)
      TeamsUser.update(team_user_mapping.id, team_id: new_team_id)
    else
      participant_id = Assignment.find(assignment_id).participants.find_by(user_id: invited_user_id).id
      TeamsUser.create(team_id: new_team_id, participant_id: participant_id)
    end
  end
