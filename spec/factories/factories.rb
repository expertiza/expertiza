FactoryBot.define do
  ...
  factory :mentor, class: AssignmentParticipant do
    sequence(:name)  { |n| "mentor206#{(n % 3) + 4}" }
    can_submit       { true }
    can_review       { true }
  ...
  factory :student, class: User do
    sequence(:name)     { |n| "student206#{(n % 3) + 4}" }
    role                { Role.where(name: 'Student').first || association(:role_of_student) }
    password            { 'password' }
    password_confirmation { 'password' }
    sequence(:fullname) { |n| "206#{(n % 3) + 4}, student" }
    email               { 'expertiza@mailinator.com' }
    parent_id           { 1 }
    private_by_default  { false }
    mru_directory_path  { nil }
  ...
  factory :assignment, class: Assignment do
    name                          do
                                     last = Assignment.last&.id
                                     last ? "assignment#{last+1}" : 'final2'
                                   end
  ...
  factory :teams_participant do
    team        { Association(:assignment_team) }
    participant { Association(:participant) }
    duty        { Association(:duty) }
  end  
