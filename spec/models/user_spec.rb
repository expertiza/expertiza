  ...
  describe '#get_user_list' do
    ...
    context 'when current user is super admin and searches by incorrect user name and full name' do
      it 'fetches an empty users list if user with given user name and full name is not found' do
        allow(user).to receive_message_chain("role.super_admin?") { true }
        expect(user.get_user_list("abc", " bbc", "")).to eq([])
      end
    end
    ...
    context 'when current user is super admin and search by incorrect user name, full name and email' do
      it 'fetches an empty users list if user with given user name, full name and email is not found' do
        allow(user).to receive_message_chain("role.super_admin?") { true }
        expect(user.get_user_list("abc", " bbc", "abcbbe@gmail.com")).to eq([])
      end
    end
  end
