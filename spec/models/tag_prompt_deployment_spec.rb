describe TagPromptDeployment do
  ...
  let(:team_participant1) { TeamsUser.new(user_id: user1.id, team_id: team.id) }
  let(:team_participant2) { TeamsUser.new(user_id: user2.id, team_id: team.id) }
  ...
  describe 'assignment_tagging_progress' do
    ...
    it 'does not vary by round' do
      ...
      allow(TeamsUser).to receive(:where).with(team_id: team.id).and_return([team_participant1, team_participant2])
