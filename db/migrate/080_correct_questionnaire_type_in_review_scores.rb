class CorrectQuestionnaireTypeInReviewScores < ActiveRecord::Migration
  def self.up
    review_rubric = ApplicationRecord.connection.select_one("select * from questionnaire_types where name = 'Review Rubric'")          
    
    if review_rubric 
      execute "update `review_scores` set questionnaire_type_id = "+review_rubric["id"].to_s+" where questionnaire_type_id is null"    
    end
    author_feedback = ApplicationRecord.connection.select_one("select * from questionnaire_types where name = 'Author Feedback'")              
    if author_feedback
