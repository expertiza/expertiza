...
=end
      @match=check_sub_rev(reviewState,submState)
      return @match
    end
  ...
  def get_relations_for_review_submission_tokens(token, stem, pos)
    ...
    if(!lemma.nil? and lemma != "" and !lemma.synsets.nil?)
      ...
      for g in 0..lemma.synsets.length - 1
        ...
        syn_arr=get_all_nyms(lemma_synset,"&")
        hyper_arr=get_all_nyms(lemma_synset,"@")
        hypo_arr=get_all_nyms(lemma_synset,"~")
        anto_arr=get_all_nyms(lemma_synset,"!")
=begin
      #synonyms
...
#-----------------------------------------------------------------------------
  def get_all_nyms(lemma_synset,opr)
#error handling for lemmas's without synsets that throw errors! (likely due to the dictionary file we are using)
    x_arr=Array.new
    lemma_x = lemma_synset.get_relation(opr)
    if(!lemma_x.nil? and lemma_x.length != 0)
      # puts "lemmaSyns.length #{lemmaSyns.length}"
      #for each synset get the values and add them to the array
      for h in 0..lemma_x.length - 1
        # puts "lemmaSyns[h].words.class #{lemmaSyns[h].words.class}"
        x_arr = x_arr + lemma_x[h].words
      end
    else
      x_arr << nil #setting nil when no synset match is found for a particular type of relation
    end
  rescue
    x_arr << nil
  end
#-------------------------------------------------------------------------------
  def check_sub_rev(reviewState,submState)
    match=0
    if(reviewState.equal?(submState))
      match = match + EXACT
    elsif(!reviewState.equal?(submState))
