module CoursesHelper
  ...
  def associated_courses(user)
    ...
    when Role.instructor.id
      ...
      # ta should be able to see all the courses they are ta-ing
    when Role.ta.id
      ...
      # student should be able to see the courses that they participate in
      # to be safe we are not assuming that all assignment participants are in courses participant
      # and all assignment team participants are in assignment participants
    when Role.student.id
      # find all courses that the student participate in
      course_id_list = []
