class StudentReviewController < ApplicationController
  ...
  def grouplist
    ...
    @review_mappings = ReviewResponseMap.where(reviewer_id: @participant.id).where(reviewee_id: @student.team.id)
    if @review_mappings.empty?
      review_mapping = ReviewResponseMap.create(reviewee_id: @student.team.id, reviewer_id: @participant.id, reviewed_object_id: @participant.assignment.id)
      @review_mappings = ReviewResponseMap.where(reviewer_id: @participant.id).where(reviewee_id: @student.team.id)
    end
    # if it is an calibrated assignment, change the response_map order in a certain way
    @review_mappings = @review_mappings.sort_by {|mapping| mapping.id % 5 } if @assignment.is_calibrated == true
