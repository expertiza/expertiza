class SurveyDeploymentsController < ApplicationController
  include SurveyDeploymentsHelper
  ...
  # This delete does not test if any response_map or responses has been created.
  ...
  def pending_surveys
    ...
    @surveys = [] # Get all the courses survey deployments for this user
    [CourseParticipant, AssignmentParticipant].each do |participant_type| # Get all the participant(courses or assignment) entries for this user
  ...
  # This method should be moved to survey_deployments_controller.rb
  def view_responses
    sd = SurveyDeployment.find(params[:id])
