class UpdateRubricTypes < ActiveRecord::Migration
  def self.up
    metareview_type = ApplicationRecord.connection.select_one("select * from `questionnaire_types` where name = 'Metareview'")
    ...
    questionnaires.each{
      ...
      questionnaire.questions.each{
         ...
         scores.each{
            | score | 
            metareview = ApplicationRecord.connection.select_one("select * from `review_of_reviews where id = #{score.instance_id}")
            if metareview != nil
