describe Team do
  ...
  let(:team_participant) { build(:team_participant, id: 1, user: user) }
  before(:each) do
    allow(TeamsUser).to receive(:where).with(team_id: 1).and_return([team_participant])
  end
  ...
  describe '#delete' do
    it 'deletes the current team and related objects and return self' do
      allow(TeamsUser).to receive_message_chain(:where, :find_each).with(team_id: 1).with(no_args).and_yield(team_participant)
    ...
    context 'when parameterized user did not join in current team yet' do
      context 'when current team is not full' do
        it 'does not raise an error' do
          ...
          allow(TeamsUser).to receive(:create).with(user_id: 1, team_id: 1).and_return(team_participant)
  ...
  describe '#copy_members' do
    it 'copies members from current team to a new team' do
      allow(TeamsUser).to receive(:create).with(team_id: 2, user_id: 1).and_return(team_participant)
      allow(Assignment).to receive(:find).with(1).and_return(assignment)
