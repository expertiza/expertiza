...
class UserTest < ActiveSupport::TestCase
  ...
  def test_check_email
    ...
    assert !ActionMailer::Base.deliveries.empty?         # Check if the mail has been queued in the delivery queue

    assert_equal [user.email], email.to                  # Check if the mail is being sent to proper user
    assert_equal "Test Email", email.subject             # Check if the mail subject is the same
