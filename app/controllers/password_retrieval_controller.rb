    ...
    if params[:user][:email].nil? or params[:user][:email].strip.length == 0
      ...
      if user
       password =  user.reset_password         # the password is reset
        MailerHelper::send_mail_to_user(user,"Your Expertiza password has been reset","send_password",password)   # MailerHelper which sends the mail to the user with the reset password.
