module AccessHelper
  ...
  def flash_msg
    if params[:controller] == 'response'
      if params.key?(:map_id)
        response_map = ResponseMap.find_by(id: params[:map_id])
      ...
      flash[:error] = "One of your teammates has started this review." if params.key?(:id) && params[:action] == 'new'
    else
