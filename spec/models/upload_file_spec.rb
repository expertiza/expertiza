describe 'UploadFile' do
  ...
  describe '#edit' do
    it 'returns the html ' do
      html = uf.edit(0).to_s
      expect(html).to eq('<tr><td align="center"><a rel="nofollow" data-method="delete" href="/items/1">Remove</a></td><td><input size="6" value="1.0" name="item[1][seq]" id="item_1_seq" type="text"></td><td><textarea cols="50" rows="1" name="item[1][txt]" id="item_1_txt" placeholder="Edit item content here">test txt</textarea></td><td><input size="10" disabled="disabled" value="UploadFile" name="item[1][type]" id="item_1_type" type="text"></td><td><!--placeholder (UploadFile does not need weight)--></td></tr>')
    end
  end

  describe '#view_item_text' do
    it 'returns the html ' do
      html = uf.view_item_text.to_s
