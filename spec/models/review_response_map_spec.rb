#OODD Project 3 OSS 2018
#Authors Piyush Mishra, Raveena D'Costa, Sahej Saluja
require 'rspec'
require 'spec_helper'
      ...
      context 'when the participant is not nil' do
        ...
        context 'when reviewee has a team' do
          it "creates a review response map" do
            ...
            temp_map = double('ReviewResponseMap', reviewed_object_id: 1, reviewer_id: 4, reviewee_id: 2, calibrate_to: false)

            allow(AssignmentTeam).to receive(:team).and_return(reviewee_team)
            allow(ReviewResponseMap).to receive(:find_by).and_return(temp_map)
  ...
  describe ".review_response_report" do
    ...
    context "when the user is not nil" do
      it "gives reviewers users' full name" do
        ...
        temp_user_ids = double('user_ids')#Mocking user ids
        temp_id = double('id', id: 1)
        temp_type = double('type', type: 'type')
  ...
  describe '.prepare_final_review_versions' do
    context 'if the round number is greater than 1' do
      it "returns updated version of review" do
        ...
        expect(ReviewResponseMap.prepare_final_review_versions(assignment, temp_map)).to eql(:"review round1" => {questionnaire_id: nil, response_ids: []}, :"review round2" => {questionnaire_id: nil, response_ids: []})
      end
    end
    ...
    context 'if the round number is not greater than 1' do
      it "returns latest version of review" do
        ...
        expect(ReviewResponseMap.prepare_final_review_versions(temp_assignment, temp_map)).to eql(review: {questionnaire_id: 2, response_ids: []})
      end
    end
  end
