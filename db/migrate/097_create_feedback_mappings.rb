class CreateFeedbackMappings < ActiveRecord::Migration
  def self.up    
    ...
    records = ApplicationRecord.connection.select_all("select * from `review_feedbacks`")
    
    records.each{
       | feedback | 
       review = ApplicationRecord.connection.select_one("select * from `reviews` where id = #{feedback["review_id"]}")
       reviewmap = ApplicationRecord.connection.select_one("select * from `review_mappings` where id = #{review["review_mapping_id"]}")
       ...
       unless reviewer.nil? || reviewee.nil?
         ...
         map = ApplicationRecord.connection.select_one("select * from `feedback_mappings` where id = (select max(id) from `feedback_mappings`)")
