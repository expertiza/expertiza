...
# ------------------------------------------------------------------------------
if PR_TITLE =~ /E[0-9]{4}/ and LoC < 50
  SMALL_PR_MESSAGE =
    markdown <<-MARKDOWN
...
# ------------------------------------------------------------------------------
if PR_TITLE.include? "WIP" or PR_TITLE.include? "wip"
  WIP_MESSAGE =
    markdown <<-MARKDOWN
...
# ------------------------------------------------------------------------------
if ADDED_FILES.grep(/.*temp.*/).any? or
   ADDED_FILES.grep(/.*tmp.*/).any? or
...
(ADDED_FILES + MODIFIED_FILES + RENAMED_FILES).each do |file|
  ...
  num_of_expectation_not_focus_on_real_value = added_lines_arr.count{ |loc| loc.scan(/\s*expect\s*[\(\{]/).count > 0 and loc.scan(/\.(to|not_to|to_not)\s*(be_nil|be_empty|eq 0|eql 0|equal 0)/).count > 0 }
