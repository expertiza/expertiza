...
class CoursesController < ApplicationController
  ...
  # if private is set to 1, then the courses will
  # only be available to the instructor who created it.
  def new
    @private = params[:private]
  end

  # Modify an existing courses
  def edit
  ...
  def update
    ...
    @course.save
    undo_link("The courses \"#{@course.name}\" has been updated successfully.")
  ...
  def copy
    ...
    begin
      ...
      undo_link("The courses \"#{orig_course.name}\" has been successfully copied.
        The copy is currently associated with an existing location from the original courses.
      ...
      flash[:error] = 'The courses was not able to be copied: ' + $ERROR_INFO
      redirect_to controller: 'tree_display', action: 'list'
    end
  end
  ...
  def create
    ...
    begin
      ...
      undo_link("The courses \"#{@course.name}\" has been successfully created.")
      redirect_to controller: 'tree_display', action: 'list'
    rescue StandardError
      flash[:error] = $ERROR_INFO # "The following error occurred while saving the courses: #"+
  ...
  def delete
    ...
    @course.destroy
    undo_link("The courses \"#{@course.name}\" has been successfully deleted.")
    redirect_to controller: 'tree_display', action: 'list'
  end
  ...
  def add_ta
    ...
    if @user.nil?
      ...
      flash.now[:error] = "The user inputted \"" + params[:user][:name] + "\" is already a TA for this courses."
    else
