...
class SentenceState
  ...
  def break_at_coordinating_conjunctions(str_with_pos_tags)
    ...
    if(str_with_pos_tags.include?("CC"))
      ...
      for i in (0..count-1)
        phrase = sen[i]
        if !phrase.nil?
          if phrase.include?("CC")
            get_array(counter,temp)
            counter += 1
            temp = phrase[0..phrase.index("/")]
            #the CC or IN goes as part of the following sentence
          else
            temp = temp +" "+ phrase[0..phrase.index("/")]
          end

        end
      end

      if temp #setting the last sentence segment
        get_array(counter,temp)
      end

    else
      get_array(counter,str_with_pos_tags)
  ...
  def get_array(a,temp_array)
    @broken_sentences[a] = temp_array
    a += 1
  end
      #------------------------------------------#------------------------------------------
  ...
  def sentence_state(str_with_pos_tags)
    ...
    for k in (0..sen.length-1)
      phrase = sen[k]
      #setting the tagged string
      tagged_tokens[i] = phrase
      if(phrase.include?("/"))
        phrase = phrase[0..phrase.index("/")-1]
      end
      #removing punctuations 
      if(phrase.include?("."))
        tokens[i] = phrase[0..phrase.index(".")-1]
      elsif(phrase.include?(","))
        tokens[i] = phrase.gsub(",", "")
      elsif(phrase.include?("!"))
        tokens[i] = phrase.gsub("!", "")
      elsif(phrase.include?(";"))
        tokens[i] = phrase.gsub(";", "")
