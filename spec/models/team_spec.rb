describe Team do
    ...
    context 'when parameterized user did not join in current team yet' do
      context 'when current team is not full' do
        it 'does not raise an error' do
          ...
          allow(TeamsParticipant).to receive(:create).with(user_id: 1, team_id: 1).and_return(team_participant)
  ...
  describe '#copy_members' do
    it 'copies members from current team to a new team' do
      allow(TeamsParticipant).to receive(:create).with(team_id: 2, user_id: 1).and_return(team_participant)
      allow(Assignment).to receive(:find).with(1).and_return(assignment)
