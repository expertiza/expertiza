describe Duty do
  ...
  before(:each) do
    ...
    allow(participant).to receive(:duty_id).and_return(nil)
    allow(participant2).to receive(:duty_id).and_return(nil)
    allow(participant3).to receive(:duty_id).and_return(1)
