describe 'bookmark review testing' do
  ...
  before(:each) do
    ...
    create(:team_participant, user: User.where(role_id: 1).first)
    create(:team_participant, user: User.where(role_id: 1).second)
    create(:assignment_team)
    create(:team_participant, user: User.where(role_id: 1).third, team: AssignmentTeam.second)
