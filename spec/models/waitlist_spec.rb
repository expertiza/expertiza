describe Waitlist do
  ...
  describe '#remove_from_waitlists' do
    it 'destroys signed up teams' do
      ...
      allow(SignedUpTeam).to receive(:where).with(topic_id: 1, is_waitlisted: false).and_return([signedupteam2, signedupteam3])
      ...
      expect(SignUpTopic).to receive(:assign_to_first_waiting_team).with(signedupteam4)
