...
describe 'SurveyDeployment' do
  ...
  describe '#response_maps' do
    it 'should return the associated response map' do
      survey_deployment = AssignmentSurveyDeployment.new(questionnaire_id: 986, start_date: previous_day, end_date: nil, parent_id: "12345678", type: "AssignmentSurveyDeployment")
      allow(AssignmentSurveyResponseMap).to receive(:where).with(reviewee_id: 986).and_return([response_map])
