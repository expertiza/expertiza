...
describe User do
  ...
  describe '#get_available_users' do
    ...
    it 'returns the first 10 visible users' do
      ...
      allow(@role).to receive(:get_parents).and_return(%w(Instructor Administrator))
  ...
  describe '#can_impersonate?' do
    ...
    it 'can impersonate target user if current user is the TA of target user' do
      ...
      allow(user1).to receive(:teaching_assistant_for?).and_return(true)
      expect(user1.can_impersonate?(user)).to be true
    end

    it 'can impersonate target user if current user is the recursively parent of target user' do
      allow(user1).to receive_message_chain("role.super_admin?") { false }
      allow(user1).to receive(:teaching_assistant_for?).and_return(false)
      allow(user1).to receive(:recursively_parent_of).and_return(true)
      expect(user1.can_impersonate?(user)).to be true
    end
  ...
  describe '.export_fields' do
    it 'exports all information setting in options' do
      options = {"personal_details" => "true",
                 ...
                 "email_options" => "true",
                 "handle" => "true"}
      expect(User.export_fields(options)).to eq(["name",
                                                 "full name",
                                                 "email",
                                                 "role",
                                                 "parent",
