class CreateAssignmentQuestionnaires < ActiveRecord::Migration
  def self.up
    ...
    l_records = ApplicationRecord.connection.select_all("select * from notification_limits")
  ...
  def self.make_association(model, assignment, questionnaire_id)
      begin
        ...
        l_records = ApplicationRecord.connection.select_all("select * from notification_limits where assignment_id = #{assignment.id} and questionnaire_id = #{q.id}")
        w_records = ApplicationRecord.connection.select_all("select * from questionnaire_weights where assignment_id = #{assignment.id} and questionnaire_id = #{q.id}")         
